...
class GaussWithError:
    ...
    def __direct_walk(matrix: list, free_coefs: list) -> list:
        ...
        for n_step in range(n_variables - 1):
            for n_row in range(n_step + 1, n_variables):
                ...
                matrix[n_row] = GaussWithError.__sub_lists(matrix[n_row], GaussWithError.__multiple_float_array_on_float(matrix[n_step], coef))
    ...
    def __inverse_walk(matrix: list, free_coefs: list) -> list:
        ...
        for n_var in range(n_variables)[::-1]:
            ...
            if abs(var_value) < GaussWithError.FLOAT_ACCURACY:
                var_value = Float(0.0)
    ...
    def solve(matrix: np.ndarray, frees: np.ndarray) -> np.ndarray:
        matrix = GaussWithError.__make_matrix_floats(matrix.tolist())
        frees = GaussWithError.__make_frees_floats(frees.tolist())
        matrix, frees = GaussWithError.__direct_walk(matrix, frees)
        answers = GaussWithError.__inverse_walk(matrix, frees)
        answers = GaussWithError.__round_answers(answers)
        return answers
