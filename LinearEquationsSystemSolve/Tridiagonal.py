...
class Tridiagonal:
    ...
    def __make_matrix_coefs(matrix: list, free_coefs: list) -> list:
        ...
        for n_row in range(len(matrix)):
            ...
            while len(matrix_coefs_row) < Tridiagonal.NON_ZERO_MATRIX_COEFS_ROW_LEN and index_of_matrix_elem_to_add < len(matrix[n_row]):
                matrix_coefs_row.append(matrix[n_row][index_of_matrix_elem_to_add])
                index_of_matrix_elem_to_add += 1
            
            while len(matrix_coefs_row) < Tridiagonal.NON_ZERO_MATRIX_COEFS_ROW_LEN:
                matrix_coefs_row.append(0)
    ...
    def solve(matrix: np.ndarray, frees: np.ndarray) -> np.ndarray:
        matrix_coefs = Tridiagonal.__make_matrix_coefs(matrix, frees)
        uv_matrix = Tridiagonal.__make_uv_matrix(matrix_coefs)
        answers = Tridiagonal.__make_answers(uv_matrix)
        return answers
