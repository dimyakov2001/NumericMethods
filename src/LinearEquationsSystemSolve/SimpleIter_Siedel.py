...
class SimpleIter_Siedel:
    ...
    def __start_method(matrix: list, free: list, e: float, siedel_method: bool = False) -> list:
        ...
        while not stop_flag:
            ...
            for i in range(len(vars)):
                ...
                vars[i] = x_i
            stop_flag = SimpleIter_Siedel.__need_continue(vars_prev, vars, e)
        return vars
    ...
    def solve(matrix: np.ndarray, frees: np.ndarray, e: float, siedel_method: bool=False) -> np.ndarray:
        if SimpleIter_Siedel.__metod_converges(matrix):
            return SimpleIter_Siedel.__start_method(matrix, frees, e, siedel_method=siedel_method)
        else:
            return None
