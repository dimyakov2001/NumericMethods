...
def __get_alpha(matrix: list) -> list:
    ...
    alpha = __calculate_alhpa_1(matrix)
    if alpha < 1.0:
        return alpha, __calculate_rho_1

    alpha = __calculate_alhpa_2(matrix)
    if alpha < 1.0:
        return alpha, __calculate_rho_2

    alpha = __calculate_alhpa_3(matrix)
    if alpha < 1.0:
        return alpha, __calculate_rho_3
...
def __start_method(matrix: list, free: list, e: float, alpha: float, rho_func: FunctionType, siedel_method: bool = False) -> list:
    ...
    while not stop_flag:
        ...
        for i in range(len(vars)):
            ...
            vars[i] = x_i
        stop_flag = __need_continue(vars_prev, vars, e, alpha, rho_func)
    return vars
...
def solve(matrix: np.ndarray, frees: np.ndarray, e: float, siedel_method: bool=False) -> np.ndarray:
    normal_matrix, normal_frees = __to_normal_form(matrix, frees)
    alpha, rho_func = __get_alpha(normal_matrix)
    if np.abs(alpha) > FLOAT_ACCURACY:
        return __start_method(normal_matrix, normal_frees, e, alpha, rho_func, siedel_method)
    else:
        return None
