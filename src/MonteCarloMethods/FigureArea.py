...
class FigureArea:
    ...
    def calculate(basic_square: np.ndarray, figure_borders: list, n_points: int) -> float:
        ...
        basic_square_area = FigureArea.__get_basic_square_area(basic_square_left_top_point, basic_square_right_bottom_point)

        points = [ FigureArea.__generate_point(basic_square_left_top_point, basic_square_right_bottom_point) for _ in range(n_points) ]
        points_in_borders = list(filter(lambda point: FigureArea.__point_satisfies_borders(point, figure_borders), points))
